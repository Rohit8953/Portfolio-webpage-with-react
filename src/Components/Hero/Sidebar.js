...
const Sidebar = () => {
  const { changebg } = useContext(Portfoliocontext);
      ...
      >
        {side ? (
          <IoReorderThreeOutline
            ...
            onClick={() =>{ setside(false); setsidebar(false)}}
          />
        ) : (
          <ImCross
            className="my-2 relative right-4 "
          ...
          {data4.map((data) => {
            return (
              <div>
                ...
                >
                  <p onClick={()=>setsidebar(true)} className="text-3xl text-[#0ef] hover:scale-110 transition-all duration-200 hover:text-white">
                    {data.icons}
                  </p>
                  <p>
                    {Sidebar ? (
